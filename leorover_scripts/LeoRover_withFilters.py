...
def get_anomaly(name):
    ...
    if len(parts) == 1:
        ...
        distortion, severity = parts[0], int(parts[1])
        return hendrycks.get_anomaly(name=distortion, severity=severity)
    else:
        raise Exception(f'Invalid anomaly: "{name}"')

class LineFollower:
    color_mask_config = None

    def __init__(self, modelPath=None, velocityTopic=None, videoTopic=None, filterType=None, savePath=None):
        ...
        print(filterType)
        self.anomaly = get_anomaly(filterType)
        ...
        self.img_pub = rospy.Publisher("line_follower_image", Image, queue_size=1)
    ...
    def video_callback(self, data: Image):
        ...
        currTime=time.time()
        if currTime-self.initialTime > 20:
            mutatedImage=self.anomaly(cv_img)
        else:
            #name=os.path.join(self.savePath,str(currTime)+'.png')
            mutatedImage=cv_img
        #cv2.imwrite(name,mutatedImage)
        processed_img = self.preprocess_img(mutatedImage)
        ...
        if self.img_pub:
            img_msg = self.bridge.cv2_to_imgmsg(mutatedImage, encoding="bgr8")
            self.img_pub.publish(img_msg)

        rospy.logdebug("steering: %f, %f" % (steering[1], steering[0]))
        
        self.publish_vel(steering)

    def publish_vel(self, steering):
        vel_msg = Twist()
        vel_msg.linear.x = steering[1]
        vel_msg.angular.z = steering[0]
...
if __name__ == "__main__":
    ...
    follower = LineFollower(args.model_path, args.velocity_topic, args.image_topic, args.filterType, args.savePath)
    rospy.spin()
